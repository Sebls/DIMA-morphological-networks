...
TRAINING_UPDATE_RULE: str = "pareto"
PARETO_MAX_RANKS: int = 3
PARETO_ACTIVE_SET_MIN: int = 2
PARETO_DOMINANCE_EPS: float = 0.0
...
# ### 5.1 Core layers
#
# This subsection defines the two core learnable building blocks:
#
# - **`MyOwnDilation`** (max-plus dilation over extracted patches),
# - **`SupErosions_block`** (two-map combiner with per-filter min then global sup).
#
# #### 5.1.1 `MyOwnDilation`
#
...
# ### 5.2 Model utilities
#
# Helpers to modify model parameters after creation:
#
# - **`apply_k_deactivated_structuring_elements_init`** randomly deactivates `k` patch entries per filter.
# - **`_apply_receptive_field_masks`** applies deterministic inactive positions in the two-bank RF architecture.
...
# %% [markdown]
# ### 5.4 Model registry and helpers
#
...
# ## 6. Training
#
# This section separates the standard compile/fit routine from Pareto-specific
# custom training-step logic.
#
# ### 6.1 Standard training
#
# `train_model` is the main entry point used by experiment runners.
# It performs compile/fit for all models and optionally injects Pareto logic
# when `update_rule` is `pareto` or `pareto_multi_rank`.
...
def train_model(
    ...
    *,
    ...
    pareto_max_ranks: int = PARETO_MAX_RANKS,
    pareto_active_set_min: int = PARETO_ACTIVE_SET_MIN,
    pareto_dominance_eps: float = PARETO_DOMINANCE_EPS,
    ...
    if ur not in {"standard", "pareto", "pareto_multi_rank"}:
        raise ValueError(
            f"Unsupported update_rule={update_rule!r}; use 'standard', 'pareto', or 'pareto_multi_rank'."
        )

    if ur in {"pareto", "pareto_multi_rank"}:
        if model.name != SingleSupErosionsArchitecture.slug:
            raise ValueError(
                "Pareto update rules currently support only the single_sup architecture."
            )
        _configure_single_sup_pareto_train_step(
            model,
            multi_rank=(ur == "pareto_multi_rank"),
            pareto_max_ranks=int(pareto_max_ranks),
            pareto_active_set_min=int(pareto_active_set_min),
            pareto_dominance_eps=float(pareto_dominance_eps),
        )

    # Pareto train_step currently uses eager-only control flow (.numpy() checks
    # for dynamic rank extraction), so force eager execution for Pareto modes.
    run_eagerly = ur in {"pareto", "pareto_multi_rank"}
    model.compile(
...
# %% [markdown]
# ### 6.2 Pareto update rules
#
# This subsection provides Pareto-specific internals used by
# `_configure_single_sup_pareto_train_step`:
#
# 1. dominance matrix,
# 2. rank extraction and subset unions,
# 3. rank-wise gradient fusion.
#
# #### Complexity and cost notes
#
# Let:
#
# - `K` = number of filters (structuring elements),
# - `P` = flattened filter size (e.g., `3x3 -> 9`),
# - `R` = extracted number of Pareto ranks.
#
# Main costs per batch:
#
# - **Dominance matrix:** `O(K^2 * P)` time, `O(K^2)` memory.
# - **Rank extraction from dominance:** about `O(R * K^2)` boolean operations.
# - **Subset construction from ranks:** near linear in number of rank indices.
# - **Backward passes:** each additional rank-loss adds one extra gradient pass over trainable variables.
#
# Practical implication: when `K` is large, runtime is mostly driven by
# (a) pairwise dominance and
# (b) number of additional rank losses/backward passes.

# %%
def _strict_dominance_matrix(filters: tf.Tensor, eps: float = 0.0) -> tf.Tensor:
    """D[j, k] is True when filter j strictly dominates filter k."""
    f_row = tf.expand_dims(filters, axis=1)  # (K,1,P)
    f_col = tf.expand_dims(filters, axis=0)  # (1,K,P)
    leq = tf.reduce_all(f_row <= (f_col + eps), axis=-1)
    strictly_less = tf.reduce_any(f_row < (f_col - eps), axis=-1)
    return tf.logical_and(leq, strictly_less)


def _pareto_ranks_from_dominance(
    dominance: tf.Tensor,
    max_ranks: int | None,
    active_set_min: int,
) -> list[tf.Tensor]:
    """Extract successive Pareto layers from a precomputed dominance matrix.

    Complexity:
    - Per extracted rank: O(K^2) boolean ops over the dominance matrix.
    - Total: O(R * K^2), where R is the number of extracted ranks.
    """
    k = int(dominance.shape[0])
    active = tf.ones((k,), dtype=tf.bool)
    ranks: list[tf.Tensor] = []
    rank_cap = k if max_ranks is None else max(1, int(max_ranks))

    for _ in range(rank_cap):
        if int(tf.reduce_sum(tf.cast(active, tf.int32)).numpy()) < max(1, int(active_set_min)):
            break
        has_active_dominator = tf.reduce_any(
            tf.logical_and(dominance, tf.expand_dims(active, axis=1)),
            axis=0,
        )
        minimal_mask = tf.logical_and(active, tf.logical_not(has_active_dominator))
        idx = tf.cast(tf.reshape(tf.where(minimal_mask), (-1,)), tf.int32)
        if int(tf.size(idx).numpy()) == 0:
            break
        ranks.append(idx)
        active = tf.logical_and(active, tf.logical_not(minimal_mask))

    if int(tf.reduce_sum(tf.cast(active, tf.int32)).numpy()) > 0:
        ranks.append(tf.cast(tf.reshape(tf.where(active), (-1,)), tf.int32))

    return ranks


def _suffix_unions_from_ranks(ranks: list[tf.Tensor]) -> list[tf.Tensor]:
    """Build S_k = union_{i=k..n} L_i efficiently from rank list.

    Building these unions is linear in the total number of rank-index elements
    (plus concat overhead), and avoids repeatedly concatenating long prefixes.
    """
    n = len(ranks)
    if n == 0:
        return []
    suffix: list[tf.Tensor] = [tf.constant([], dtype=tf.int32)] * n
    running = ranks[-1]
    suffix[-1] = running
    for i in range(n - 2, -1, -1):
        running = tf.concat([ranks[i], running], axis=0)
        suffix[i] = running
    return suffix


def _rank_index_map_from_ranks(ranks: list[tf.Tensor], n_filters: int) -> tf.Tensor:
    """Return map filter_index -> Pareto rank id.

    This is used to route gradients rank-wise for erosion filters. Cost is O(K)
    in the number of filter indices scattered.
    """
    rank_index_map = tf.zeros((n_filters,), dtype=tf.int32)
    for ridx, idx in enumerate(ranks):
        rank_index_map = tf.tensor_scatter_nd_update(
            rank_index_map,
            tf.expand_dims(idx, axis=-1),
            tf.fill((tf.shape(idx)[0],), tf.cast(ridx, tf.int32)),
        )
    return rank_index_map


def _single_sup_maps_model(model: keras.Model) -> keras.Model:
    erosions = model.get_layer("Erosion")
    return keras.Model(model.input, -erosions.output)


def _subset_sup_from_maps(erosion_maps: tf.Tensor, indices: tf.Tensor) -> tf.Tensor:
    submaps = tf.gather(erosion_maps, indices, axis=-1)
    return tf.reduce_max(submaps, axis=-1, keepdims=True)


def _fuse_rank_gradients_for_single_sup(
    var: tf.Variable,
    grads_per_rank: list[tf.Tensor | None],
    rank_index_map: tf.Tensor,
) -> tf.Tensor | None:
    full_grad = grads_per_rank[0]
    if full_grad is None:
        return None
    if not var.name.endswith("Erosion/variable:0"):
        return full_grad

    n_filters = int(var.shape[-1])
    grad = tf.zeros_like(full_grad)
    for ridx, g_rank in enumerate(grads_per_rank):
        if g_rank is None:
            continue
        mask = tf.equal(rank_index_map, ridx)
        mask_f = tf.cast(mask, dtype=g_rank.dtype)
        mask_f = tf.reshape(mask_f, (1, 1, 1, 1, n_filters))
        grad = grad + g_rank * mask_f
    return grad


def _configure_single_sup_pareto_train_step(
    model: keras.Model,
    *,
    multi_rank: bool,
    pareto_max_ranks: int | None,
    pareto_active_set_min: int,
    pareto_dominance_eps: float,
) -> None:
    maps_model = _single_sup_maps_model(model)

    def train_step(self, data):
        x, y = data
        with tf.GradientTape(persistent=True) as tape:
            erosion_maps = maps_model(x, training=True)
            y_pred_full = tf.reduce_max(erosion_maps, axis=-1, keepdims=True)
            loss_full = self.compiled_loss(y, y_pred_full, regularization_losses=self.losses)

            w = self.get_layer("Erosion").w
            filters = tf.reshape(w[0, 0, 0, :, :], (-1, tf.shape(w)[-1]))
            filters = tf.transpose(filters)
            dominance = _strict_dominance_matrix(filters, eps=pareto_dominance_eps)

            if multi_rank:
                ranks = _pareto_ranks_from_dominance(
                    dominance,
                    max_ranks=pareto_max_ranks,
                    active_set_min=pareto_active_set_min,
                )
            else:
                ranks = _pareto_ranks_from_dominance(
                    dominance,
            ...
            # S_k = union_{i=k..n} L_i (nested subsets for multi-rank losses).
            suffix_indices = _suffix_unions_from_ranks(ranks)

            losses_by_rank: list[tf.Tensor] = [loss_full]
            for s_idx in range(1, len(suffix_indices)):
                y_pred_sub = _subset_sup_from_maps(erosion_maps, suffix_indices[s_idx])
                losses_by_rank.append(self.compiled_loss(y, y_pred_sub, regularization_losses=[]))

        grads_per_rank = [
            tape.gradient(loss_k, self.trainable_variables) for loss_k in losses_by_rank
        ]

        n_filters = int(model.get_layer("Erosion").w.shape[-1])
        rank_index_map = _rank_index_map_from_ranks(ranks, n_filters)

        final_grads: list[tf.Tensor | None] = []
        for v_idx, var in enumerate(self.trainable_variables):
            per_rank_var_grads = [g[v_idx] if g is not None else None for g in grads_per_rank]
            final_grads.append(
                _fuse_rank_gradients_for_single_sup(var, per_rank_var_grads, rank_index_map)
            )

        self.optimizer.apply_gradients(zip(final_grads, self.trainable_variables))
        self.compiled_metrics.update_state(y, y_pred_full)
        results = {m.name: m.result() for m in self.metrics}
        results["loss"] = loss_full
        return results

    model.train_step = train_step.__get__(model, type(model))
...
# %%
def run_k_deactivated_single_architecture_experiment(
    out_dir: Path | str | None = None,
    *,
    notebook: bool = False,
    dated_subdir: bool = True,
    ...
    """
    Same protocol as ``run_k_deactivated_initialization_experiment`` but only
    for the single architecture (``single_sup``) across k-deactivation variants.

    Keeps the same output artifacts:
    - metrics / metadata JSON
    - weight-pair logs
    - all comparison plots (training overlays, prediction grids, etc.)
    """
    import matplotlib
    ...
    parent = Path(out_dir).expanduser()
    parent.mkdir(parents=True, exist_ok=True)
    prefix = f"exp_{EXPERIMENT_TARGET_SLUG}_single_k_deactivated_grid"
    if dated_subdir:
        run_dir = new_dated_experiment_dir(parent, prefix)
    else:
        run_dir = parent
    ...
    arch_slugs = [SingleSupErosionsArchitecture.slug]
    ks = [0, 1, 2, 3, 4] if k_values is None else [int(x) for x in k_values]
    ...
    print("Target:", target_meta["target_key"], "—", target_meta["target_label"])
    print("K-deactivated single-architecture grid:", arch_slugs, "k_values", ks)
    print("Shapes: x_train", pack["x_train"].shape, "y_train", y_train.shape)
    print("Experiment run directory:", run_dir)
    ...
    summary = {
        "protocol": "k_deactivated_single_architecture_grid",
        "run_dir": str(run_dir.resolve()),
        "architectures": arch_slugs,
        "k_values": ks,
        "variant_slugs": variant_slugs,
    ...
    summary_path = run_dir / "k_deactivated_single_architecture_summary.json"
    with open(summary_path, "w") as f:
        json.dump(summary, f, indent=2)
    ...
    if notebook:
        try:
            ...
            display(Markdown(f"**K-deactivated single-architecture run:** `{run_dir}`"))
            display(Markdown(f"**Summary:** `{summary_path}`"))
            display(Markdown(f"**Figures:** `{plots_dir}`"))
            for p in sorted(plots_dir.glob("*.png")):
                display(Image(filename=str(p)))
...
# %%
run_k_deactivated_single_architecture_experiment(
    out_dir='/content/outputs', notebook=False, dated_subdir=True
)
